# Neovim Configuration

## NNN File Explorer

Bidirectional directory sync between neovim and nnn via [nnn.nvim](https://github.com/luukvbaal/nnn.nvim).

### Keybindings

| Key | Context | Action | Mnemonic |
|-----|---------|--------|----------|
| `<C-b>` | nnn terminal | cd neovim to nnn's current directory | **b**rowse here |
| `<C-d>` | nnn terminal | show nnn's current directory as notification | **d**irectory |
| `<Leader>nf` | normal buffer | focus nnn pane (opens it if closed) | **n**nn **f**ocus |
| `<Leader>ns` | normal buffer | navigate nnn to current file's directory | **n**nn **s**ync |

### How `<Leader>ns` works

1. Neovim writes the current buffer's parent directory to `/tmp/nnn-goto`
2. Sends `;n` to the nnn terminal via `chansend`, triggering the `nvimcd` plugin
3. The plugin reads the path and calls `nnn_cd` to navigate nnn in the current context
4. nnn state (tabs, selections, etc.) is fully preserved

### Files involved

| File | Role |
|------|------|
| `~/.config/nvim/lua/plugins/nav/nnn.lua` | Plugin config, keybindings, helper functions |
| `~/.config/nnn/plugins/nvimcd` | nnn plugin that reads `/tmp/nnn-goto` and calls `nnn_cd` |
| `~/.nnn.bash` | Exports `NNN_PLUG` with `n:nvimcd` registered |
| `~/.config/nnn/plugins/.nnn-plugin-helper` | Provides `nnn_cd` (writes `0c<path>` to `NNN_PIPE`) |

### How `<C-b>` works

Reads the nnn process's current working directory from `/proc/<pid>/cwd` via
`vim.fn.jobpid()` and `vim.loop.fs_readlink()`. This bypasses nnn's mapping
system entirely (set via a neovim `FileType` autocmd on the nnn buffer).

## Markdown Preview

Two complementary preview modes for markdown files.

### In-buffer rendering

[render-markdown.nvim](https://github.com/MeanderingProgrammer/render-markdown.nvim)
styles headers, tables, checkboxes, and inline formatting directly in the
buffer. Files open in raw source mode by default.

### Floating glow preview

[glow.nvim](https://github.com/ellisonleao/glow.nvim) opens a floating
window with a fully rendered preview powered by the
[glow](https://github.com/charmbracelet/glow) CLI. Press `q` or `<Esc>`
to close the preview.

### Keybindings

| Key | Action | Mnemonic |
|-----|--------|----------|
| `<Leader>mr` | Toggle in-buffer rendered/raw source | **m**arkdown **r**ender |
| `<Leader>mg` | Open floating glow preview | **m**arkdown **g**low |

## AI Integration

Two complementary plugins for AI-assisted editing, each solving different
constraints around authentication and rate limits.

### avante.nvim — Lightweight in-editor AI chat

[avante.nvim](https://github.com/yetone/avante.nvim) provides a chat sidebar
and code editing via the Anthropic API.

**Authentication:** Reads the OAuth-managed API key from `~/.claude.json`
(generated by `claude` CLI SSO login). No separate API key needed.

**Rate limit caveat:** Direct API calls via this key use a lower rate-limit
tier than the Claude Code CLI's own OAuth path. Opus and Sonnet are typically
rate-limited; **Haiku** works reliably. If your org upgrades the API tier,
change the model in `plugins/ai/avante.lua`.

**Config migration notes (2026-04):**
- Provider config must be nested under `providers.claude` (not top-level `claude`)
- `max_tokens` goes inside `extra_request_body`
- Model IDs: use `claude-haiku-4-5-20251001`, `claude-sonnet-4-6`, `claude-opus-4-6`
  (no date suffix for 4.6 models)
- `mode = "agentic"` sends `text_editor` tool types that 4.6 models may not
  support — use `mode = "legacy"` if you hit `not_found_error` on tool types

| Key | Mode | Action |
|-----|------|--------|
| `<Leader>aa` | normal/visual | Open chat sidebar (with selection context in visual) |
| `<Leader>ae` | visual | Edit selected code with AI |
| `<Leader>ar` | normal | Regenerate response |
| `<Leader>at` | normal | Toggle sidebar visibility |
| `<Leader>af` | normal | Switch focus between sidebar and code |

### claudecode.nvim — CLI bridge (full OAuth rate limits)

[claudecode.nvim](https://github.com/coder/claudecode.nvim) bridges Neovim to
the Claude Code CLI via WebSocket (same mechanism as the VS Code extension).
All API calls go through the CLI's OAuth session — no API key needed, full org
rate limits, all models available.

See `plugins/ai/claudecode.lua` for configuration.

### Why two plugins?

| | avante.nvim | claudecode.nvim |
|---|---|---|
| **Auth** | API key (from ~/.claude.json) | OAuth via CLI |
| **Rate limits** | Lower tier (Haiku only) | Full org limits |
| **Models** | Haiku | All (Opus, Sonnet, Haiku) |
| **UX** | Native sidebar, no CLI needed | Terminal-based, CLI interaction |
| **Best for** | Quick questions, lightweight edits | Heavy tasks, code generation |
